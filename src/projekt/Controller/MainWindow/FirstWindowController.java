...
import projekt.Activity.ValidateActivity;
    ...
    private void nextWindow(ActionEvent event) throws IOException {
        ...
        if (mail) {
            if (!isEmptySurname() && !isEmptySex()) {
                try {
                    if (ValidateActivity.checkHeightCent(height.getText()) || ValidateActivity.checkHeightMetr(height.getText())) {
                        w = Double.valueOf(height.getText());
                        if (ValidateActivity.checkHeightMetr(height.getText())) {
                            w *= 100;
                        }
                        h = w.intValue();
                        w = 0.0;
                    } else {
                        showOutputMessageError("Niepoprawnie wprowadzono dane na temat wzrostu.\nWzrost podajemy w centymertrach!");
                        return;
                    }

                    if (ValidateActivity.checkWeight(weight.getText()) || ValidateActivity.checkWeight2(weight.getText())) {
                        w = Double.valueOf(weight.getText());
                    } else {
                        showOutputMessageError("Niepoprawnie wprowadzono dane na temat wagi.\nWagę podajemy w kilogramach!");
                        return;
                    }

                    if (ValidateActivity.checkAge(age.getText())) {
                        a = Integer.parseInt(age.getText());
                    } else {
                        showOutputMessageError("Niepoprawnie wprowadzono dane na temat wieku.\nWiek podajemy w latach!");
                        return;
                    }

                    Person p = new Person(surname.getText(), w, a, sex.getValue(), h);
                    if (ValidateActivity.checkEmail(surname.getText())) {
                        ...
                        showOutputMessageError("Wprowadzony adres e-mail jest nieprawidłowy.\nPrzkładowy poprawny adres e-mail: jan_kowalski@email.com");
                    }
                } catch (IOException | NumberFormatException e) {
                    showOutputMessageError("Niepoprawnie wprowadzono dane na temat wagi.\nWagę podajemy w kilogramach!");
                }
            } else {
                showOutputMessageError("Wypełnij pola które nie zostały jeszcze uzupełnione");
            }
        } else {
            if (!isEmptyName() && !isEmptySurname() && !isEmptySex()) {
                try {
                    if (ValidateActivity.checkWord(name.getText()) && ValidateActivity.checkWord(surname.getText())) {
                        if (ValidateActivity.checkHeightCent(height.getText()) || ValidateActivity.checkHeightMetr(height.getText())) {
                            w = Double.valueOf(height.getText());
                            if (ValidateActivity.checkHeightMetr(height.getText())) {
                                w *= 100;
                        ...
                        showOutputMessageError("Niepoprawnie wprowadzono imie i/lub nazwisko.\nKażde imię i nazwisko powinno zaczynać się od dużej litery\nPrzykład prawidłowego imienia: Jan \nPrzykład prawidłowego nazwiska: Nowak");
                    }
                } catch (IOException | NumberFormatException e) {
                    showOutputMessageError(e.toString());
                }
            } else {
                showOutputMessageError("Wypełnij dane, które nie zostały jeszcze uzupełnione!");
            }
        }
